        ...
        if boostrap[i].len() == 0 {
            ...
            for j in 0..boostrap[i].len() {
                routing_node.push(RoutingNode {
                    ...
                    address: vec![bootstrap_node[boostrap[i][j]].2.clone()],
                });
                // last item
    ...
    for i in 0..addresable.len() {
        for j in 0..addresable[i].len() {
            routing_node.push(RoutingNode {
                ...
                address: vec![bootstrap_node[addresable[i][j]].2.clone()],
            });
    ...
    for i in 0..ephemeral.len() {
        for j in 0..ephemeral[i].len() {
            routing_node.push(RoutingNode {
                ...
                address: vec![bootstrap_node[ephemeral[i][j]].2.clone()],
            });
...
async fn create_and_test_nodes_with_trazabilty(
    ...
    // verify that the ledger is copied to all nodes
    loop {
        let response = vec_nodes[index_governance]
            .0
...
mod test {
    ...
    async fn addresable_get_governance() {
        ...
        let vec_nodes = nodes
            .into_iter()
            .flatten()
            .collect::<Vec<(Api, McNodeData, String)>>();
    ...
    async fn genesis_event_one_node() {
        ...
        create_and_test_nodes_with_trazabilty(vec![(api, mc_data_node1, "".to_string())], 0, 0)
            .await;
    }
