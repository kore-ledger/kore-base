...
use libp2p::{
    ...
    request_response::{self, Config as ReqResConfig, ProtocolSupport},
    swarm::behaviour::toggle::Toggle,
    swarm::NetworkBehaviour,
    Multiaddr, PeerId, StreamProtocol,
};
use tell::{
    binary, Event as TellEvent, OutboundTellId, ProtocolSupport as TellProtocol, TellMessage,
};
...
pub enum Event {
    ...
    MessageProcessed { peer_id: PeerId, inbound_id: String },
...
impl From<request_response::Event<Vec<u8>, Vec<u8>>> for Event {
    fn from(event: request_response::Event<Vec<u8>, Vec<u8>>) -> Self {
        match event {
            request_response::Event::Message { peer, message } => Event::RequestResponseMessage {
                peer_id: peer,
                message,
            },
            request_response::Event::ResponseSent { peer, request_id } => Event::MessageSent {
                peer_id: peer,
                outbound_id: request_id.to_string(),
            },
            request_response::Event::InboundFailure {
                ...
                peer_id: peer,
                inbound_id: request_id.to_string(),
                error: Error::Behaviour(error.to_string()),
            },
...
mod tests {
    ...
    async fn test_full() {
        ...
        node_b.add_external_address(node_b_addr.clone());
        node_b.behaviour_mut().set_random_walk(1);
